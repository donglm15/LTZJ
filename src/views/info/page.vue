  ...
  methods: {
    ...
    getList() {
      ...
      const filterData = this.tableData.filter(item => {
        ...
        if (type && item.infoType.id != type) return false//eslint-disable-line
        return true
      })
    ...
    filterTag(value, row) {
      if (value == 200) { return row.read >= value } else { return row.read > value && row.read < 200 }//eslint-disable-line
    },
    handleCreate() {
    ...
    checkInfo(row) { // 查看资讯详情
      this.tableData.forEach((news, idx) => {
        if (news.id == row.id) {//eslint-disable-line
          this.tempId = idx
        }
      })
