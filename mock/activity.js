...
export default [
...

  {
    ...
    type: 'get',
    response: config => {
      const { startTime, endTime, title, page, limit, type } = config.query
      const mockList = List.filter(items => {
        ...
        if (title && items.title.indexOf(title) < 0) return false
        if (type && items.activityType.type !== type) return false
        return true
      })
